...
describe('isOdd(number)', () => {
	...
	it('should throw a TypeError if num is not type of Number', () => {
		const string = 'i am a string';
		const object = { i: 'am', an: 'object' };
		const array = ['i', 'am', 'an', 'array'];

		expect(() => isOdd(string)).to.throw(TypeError);
		expect(() => isOdd(object)).to.throw(TypeError);
		expect(() => isOdd(array)).to.throw(TypeError);
	});
});
